import { T as TSS_SERVER_FUNCTION, a as createServerFn } from "./server-ztKtDdl1.mjs";
...
const FROM_EMAIL = process.env.FROM_EMAIL || "la-mama@wearesignalzero.tech";
...
const createBooking = createServerFn({
  method: "POST"
}).validator((input) => BookingSchema.parse(input)).handler(createBooking_createServerFn_handler, async ({
  data
}) => {
  const bookingId = randomUUID();
  const dateLabel = (/* @__PURE__ */ new Date(`${data.appointment_date}T${data.appointment_time}`)).toLocaleString("de-DE", {
    weekday: "long",
    day: "2-digit",
    month: "long",
    year: "numeric",
  ...
  const emailHtml = `
    ...
    `;
  tasks.push(fetch("https://api.brevo.com/v3/smtp/email", {
    method: "POST",
    headers: {
  ...
  const telegramText = `🪒 <b>Neue Buchung!</b>
...
<b>Email:</b> ${escapeHtml(data.email)}` + (data.notes ? `
